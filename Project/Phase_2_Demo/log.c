...
void printLogAddr(LogAddress logAddress1){
	cout << "segment: " << logAddress.segment << ", block: " << logAddress.block << endl;
}
...
int loadSegment(u_int segmentNum, Segment* segment){
	...
	cout << "Loading segment " << segmentNum << " into segment cache" << endl; 
	...
	cout << "Loading segment summary for segment " << segmentNum << endl;
...
int writeTailSegToFlash(){
	...
	cout << "Flushing tail segment to flash" << endl;
	...
	cout << "Flushed tail segment " << this_segment << " to flash" endl;
...
int writeToTail(u_int inum, u_int block, u_int length, const void *buffer, LogAddress *logAddress){
	...
	cout << "Writing to tail segment at " << endl;
	printLogAddr(logAddress);
...
void printSuperBlock(){
	...
	cout << "flash filename: " << superBlock.fileName << endl;
	cout << "segment size: " << superBlock.segment_size << endl;
	cout << "block size: " << superBlock.block_size << endl;
	cout << "num segments: " << superBlock.num_segments << endl;
	cout << "wearlimit: " << superBlock.wearLimit << endl;
	cout << "checkpoint addresses: " << superBlock.cr_addresses[0]
		<< " , " << superBlock.cr_addresses[1] << endl; 
...
void printCheckPoints(){
	for(int i=0; i<2; i++){
		...
		cout << "Write Time: " << cr_array[i].write_time;
		cout << "Last segment written: " << cr_array[i].lastSegmentAddress;
	}
}
...
void printSegmentSummary(Segment *segment){
	SegSummary *segSummary = &(segment->segSummary);
	cout << "Segment num: " << segSummary->this_segment;
	cout << "Next segment num: " << segSummary->next;

	for(int i =0 ; i<superBlock.segment_size; i++){
		cout << "Block summary: " << i << endl;
		cout << "inum: " << segSummary->blockInfos[i].inum << endl;
		cout << "block offset: " << segSummary->blockInfos[i].blockInfos << endl;
	}
}

void readInum_blkOffset(LogAddress logAddress, u_int *inum, u_int *block_offset){
...
int Log_Open(bool isFlashEmpty){
	...
	*/
	...
	// Perform many log writes successively and read using Log reads

	LogAddress logAddress1, logAddress2, logAddress3, logAddress4;
	LogAddress logAddress5, logAddress6, logAddress7, logAddress8;

	u_int w_size1, w_size2, w_size3, w_size4;

	char w_str1[] = "First string";
	char w_str2[] = "Second string";
	char w_str3[] = "Third string";
	char w_str4[] = "Fourth string";

	char w_str5[] = "Fifth string";
	char w_str6[] = "Sixth string";
	char w_str7[] = "Seventh string";
	char w_str8[] = "Eighth string";

	w_size1 = sizeof(w_str1);
	w_size2 = sizeof(w_str2);
	w_size3 = sizeof(w_str3);
	w_size4 = sizeof(w_str4);

	w_size5 = sizeof(w_str5);
	w_size6 = sizeof(w_str6);
	w_size7 = sizeof(w_str7);
	w_size8 = sizeof(w_str8);

	Log_Write(20, 2, w_size1, w_str1, &logAddress1);
	cout << "Wrote the string: " << w_str1 << " at " << endl;  
	printLogAddr(logAddress1);

	Log_Write(10, 3, w_size2, w_str2, &logAddress2);
	cout << "Wrote the string: " << w_str2 << " at " << endl;  
	printLogAddr(logAddress2);

	Log_Write(15, 1, w_size3, w_str3, &logAddress3);
	cout << "Wrote the string: " << w_str3 << " at " << endl;  
	printLogAddr(logAddress3);

	Log_Write(4, 0, w_size4, w_str4, &logAddress4);
	cout << "Wrote the string: " << w_str4 << " at " << endl;  
	printLogAddr(logAddress4);

	// Printing the segment summary before flushing the tail segment to flash
	cout << "Printing the segment summary before flushing the tail segment to flash" << endl;
	printSegmentSummary(&segmentCache[tailSegIndex]);

	// Flush to flash
	writeTailSegToFlash();

	Log_Write(17, 3, w_size5, w_str5, &logAddress5);
	cout << "Wrote the string: " << w_str5 << " at " << endl;  
	printLogAddr(logAddress5);

	Log_Write(8, 3, w_size6, w_str6, &logAddress6);
	cout << "Wrote the string: " << w_str6 << " at " << endl;  
	printLogAddr(logAddress6);

	// Flush to flash
	writeTailSegToFlash();

	Log_Write(6, 2, w_size7, w_str7, &logAddress7);
	cout << "Wrote the string: " << w_str7 << " at " << endl;  
	printLogAddr(logAddress7);

	Log_Write(3, 1, w_size8, w_str8, &logAddress8);
	cout << "Wrote the string: " << w_str8 << " at " << endl;  
	printLogAddr(logAddress8);


	/********************/
	// Read back the written strings along with the inum and block offset for the block
	u_int inum;
	u_int block_offset;

	char *r_str1 = malloc(w_size1);
	char *r_str2 = malloc(w_size2);
	char *r_str3 = malloc(w_size3);
	char *r_str4 = malloc(w_size4);
	char *r_str5 = malloc(w_size5);
	char *r_str6 = malloc(w_size6);
	char *r_str7 = malloc(w_size7);
	char *r_str8 = malloc(w_size8);

	/*** Str1 ***/

	Log_Read(logAddress1, w_size1, r_str1);
	cout << "Read the string: " << r_str1 << " at " << endl;
	printLogAddr(logAddress1);

	// Also read the inum and block offset from segment summary for the address
	// readInum_blkOffset(logAddress1, &inum, &block_offset);
	// cout << "inum: " << inum << " block_offset: " << block_offset << endl;

	/*** Str2 ***/

	Log_Read(logAddress2, w_size2, r_str2);
	cout << "Read the string: " << r_str2 << " at " << endl;
	printLogAddr(logAddress2);

	// Also read the inum and block offset from segment summary for the address
	// readInum_blkOffset(logAddress2, &inum, &block_offset);
	// cout << "inum: " << inum << " block_offset: " << block_offset << endl;

	/*** Str3 ***/

	Log_Read(logAddress3, w_size3, r_str3);
	cout << "Read the string: " << r_str3 << " at " << endl;
	printLogAddr(logAddress3);

	// Also read the inum and block offset from segment summary for the address
	// readInum_blkOffset(logAddress3, &inum, &block_offset);
	// cout << "inum: " << inum << " block_offset: " << block_offset << endl;

	/*** Str4 ***/

	Log_Read(logAddress4, w_size4, r_str4);
	cout << "Read the string: " << r_str4 << " at " << endl;
	printLogAddr(logAddress4);

	// Also read the inum and block offset from segment summary for the address
	// readInum_blkOffset(logAddress4, &inum, &block_offset);
	// cout << "inum: " << inum << " block_offset: " << block_offset << endl;	

	/*** Str5 ***/

	Log_Read(logAddress5, w_size5, r_str5);
	cout << "Read the string: " << r_str5 << " at " << endl;
	printLogAddr(logAddress5);

	// Also read the inum and block offset from segment summary for the address
	// readInum_blkOffset(logAddress5, &inum, &block_offset);
	// cout << "inum: " << inum << " block_offset: " << block_offset << endl;

	/*** Str6 ***/

	Log_Read(logAddress6, w_size6, r_str6);
	cout << "Read the string: " << r_str6 << " at " << endl;
	printLogAddr(logAddress6);

	// Also read the inum and block offset from segment summary for the address
	// readInum_blkOffset(logAddress6, &inum, &block_offset);
	// cout << "inum: " << inum << " block_offset: " << block_offset << endl;

	/*** Str7 ***/

	Log_Read(logAddress7, w_size7, r_str7);
	cout << "Read the string: " << r_str7 << " at " << endl;
	printLogAddr(logAddress7);

	// Also read the inum and block offset from segment summary for the address
	// readInum_blkOffset(logAddress7, &inum, &block_offset);
	// cout << "inum: " << inum << " block_offset: " << block_offset << endl;

	/*** Str8 ***/

	Log_Read(logAddress8, w_size8, r_str8);
	cout << "Read the string: " << r_str8 << " at " << endl;
	printLogAddr(logAddress8);

	// Also read the inum and block offset from segment summary for the address
	// readInum_blkOffset(logAddress8, &inum, &block_offset);
	// cout << "inum: " << inum << " block_offset: " << block_offset << endl;
...
int writeCheckpoint(){
	...
	cout << "writing checkpoint at index " << indexToWrite << endl;
