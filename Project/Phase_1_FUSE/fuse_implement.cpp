...
// Fuse_Implement fuse_implement;

// Initialize LFS directory layer. This will create a root directory "/".
static int Fuse_Implement::imp_init(struct fuse_conn_info *conn)
{
...
static int Fuse_Implement::imp_file_getattr(const char* path, struct stat*)
{
    // NOT IMPLEMENTED YET
    return NOTSUPPORTED;
}

static int Fuse_Implement::imp_file_open(const char* path, struct fuse_file_info* fi)
{
    std::cout << "Opening a file in imp_file_open" << std::endl;
    return directory->Directory_file_open(path, fi);
}

static int Fuse_Implement::imp_file_read(const char* path, char* buffer, int length, off_t offset, struct fuse_file_info* fi)
{
    std::cout << "File read in imp_file_read" << std::endl;
    return directory->Directory_file_read(path, buffer, offset, length);
}

static int Fuse_Implement::imp_file_write(const char* path, void* buffer, off_t offset, int length, struct fuse_file_info* fi)
{
    std::cout << "File write in imp_file_write" << std::endl;
    return directory->Directory_file_write(path, buffer, offset, length);
}

static int Fuse_Implement::imp_unlink(const char* path)
{
    std::cout << "Unlinking a file from the directory. imp_unlink." << std::endl;
    return unlink(path);
}

static int Fuse_Implement::imp_rmdir(const char* path)
{
    // NOT IMPLEMENTED YET
...
static int Fuse_Implement::imp_statfs(const char* path, struct statvfs* stbuf)
{
    std::cout << "Return statistics about the filesystem. imp_statfs" << std::endl;
    return statvfs(path, statvfs);
}

static int Fuse_Implement::imp_file_release(const char* path, struct fuse_file_info *fi)
{
...
static int Fuse_Implement::imp_mkdir(const char* path, mode_t mode)
{
    std::cout << "mkdir. imp_mkdir" << std::endl;
    return directory->Directory_Create(path, mode);
}

static int Fuse_Implement::imp_dir_open(const char* path, struct fuse_file_info* fi)
{
    std::cout << "Open directory. dir_open" << std::endl;
    return directory->Directory_open(path, fi);
}

static int Fuse_Implement::imp_dir_read(const char* path, fuse_fill_dir_t filler, void* buffer, off_t offset, struct fuse_file_info* fi)
{
...
static int Fuse_Implement::imp_dir_release(const char* path, struct fuse_file_info* fi)
{
...
static int Fuse_Implement::imp_destroy(void *private_data)
{
...
static int Fuse_Implement::imp_file_create(const char* path, mode_t mode, struct fuse_file_info* fi)
{
    std::cout << "Create a file. imp_file_create" << std::endl;
    return directory->Directory_file_create(path, fi);
}

static int Fuse_Implement::imp_link(const char* src_path, const char* dest_path)
{
    std::cout << "Create a hard link between src_path and dest_path." << std::endl;
    return NOTSUPPORTED;
}

static int Fuse_Implement::imp_symlink(const char* src_path, const char* dest_path)
{
...
static int Fuse_Implement::imp_truncate(const char* path, int size)
{
...
static int Fuse_Implement::imp_rename(const char* org_path, const char* new_path, unsigned int flags)
{
    std::cout << "Rename a file. imp_rename" << std::endl;
    return Directory->Directory_file_rename(org_path, new_path);
}

static int Fuse_Implement::imp_chmod(const char * path, mode_t mode, struct fuse_file_info *fi)
{
    std::cout << "Change mode. imp_chmod" << std::endl;
    return Directory->Directory_chmod(path, mode, fi);
}

static int Fuse_Implement::imp_chown(const char * path, uid_t uid, gid_t id, struct fuse_file_info *fi)
{
